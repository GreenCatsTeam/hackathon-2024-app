...
class APIService {
  ...
  //static Future<bool> sendTrash(sendTrashModel model) async
  //{
    ...
    //var url = Uri.http(Config.apiURL, Config.postCards);
  ...
  static Future<String?> sendImage(String image) async
  {Map<String, String> requestHeaders = {
    'Content-type': 'application/json',
    'Host': Config.flaskURL
    };
  var url = Uri.http(Config.apiURL, Config.loginAPI);



  requestHeaders['Content-Length'] = jsonEncode(toJsons(image)).length.toString();
  print(jsonEncode(toJsons(image)));
  print(requestHeaders.toString());
  print(url);
  print("yay!");

  var responce = await client.post(url, headers: requestHeaders, body: jsonEncode(toJsons(image)));
